...
class DatabaseService {
  ...
  // Cria uma nova meta
  Future<String> createMeta(String uid, Meta meta) async {
    final metaRef = _database.child('usuarios').child(uid).child('metas').push();
    await metaRef.set(meta.toMap());
    return metaRef.key!;
  }

  // Atualiza uma meta existente
  Future<void> updateMeta(String uid, String metaId, Meta meta) async {
    await _database
        .child('usuarios')
        .child(uid)
        .child('metas')
  ...
  // Deleta uma meta
  Future<void> deleteMeta(String uid, String metaId) async {
    await _database
        .child('usuarios')
        .child(uid)
        .child('metas')
  ...
  Future<List<Meta>> getMetas(String uid) async {
    final snapshot = await _database.child('usuarios').child(uid).child('metas').get();
    if (snapshot.exists) {
      final Map<dynamic, dynamic> metasMap = Map<dynamic, dynamic>.from(snapshot.value as Map);
      return metasMap.entries
          .map((entry) => Meta.fromMap(entry.key as String, Map<String, dynamic>.from(entry.value)))
          .toList();
    }
    return [];
  }
  ...
  Future<Meta?> getMetaPorId(String uid, String metaId) async {
    final snapshot = await _database.child('usuarios').child(uid).child('metas').child(metaId).get();
    if (snapshot.exists) {
      return Meta.fromMap(metaId, Map<String, dynamic>.from(snapshot.value as Map));
    }
    return null;
  }

  // Stream de metas (para atualizações em tempo real)
  Stream<List<Meta>> getMetasStream(String uid) {
    return _database
        .child('usuarios')
        .child(uid)
        .child('metas')
        ...
        .map((event) {
      if (event.snapshot.exists) {
        final Map<dynamic, dynamic> metasMap = Map<dynamic, dynamic>.from(event.snapshot.value as Map);
        return metasMap.entries
            .map((entry) => Meta.fromMap(entry.key as String, Map<String, dynamic>.from(entry.value)))
            .toList();
      }
  ...
  // Calcula o valor atual de um indicador dentro de um período específico
  Future<double> calcularValorAtualIndicador(
    String uid,
    String indicador, {
    DateTime? dataInicio,
    DateTime? dataFim,
  }) async {
    // Define período padrão se não especificado (tudo até hoje)
    final periodoInicio = dataInicio ?? DateTime(2000, 1, 1);
    final periodoFim = dataFim ?? DateTime.now();
    
    // Normaliza as datas para comparação (remove horas)
    final inicioPeriodo = DateTime(periodoInicio.year, periodoInicio.month, periodoInicio.day);
    final fimPeriodo = DateTime(periodoFim.year, periodoFim.month, periodoFim.day, 23, 59, 59);
    
    // Função auxiliar para verificar se uma data está no período (inclusive nas bordas)
    bool estaNoPeriodo(DateTime data) {
      final dataNormalizada = DateTime(data.year, data.month, data.day);
      // Compara as datas normalizadas diretamente
      final compareInicio = dataNormalizada.compareTo(inicioPeriodo);
      final compareFim = dataNormalizada.compareTo(fimPeriodo);
      // Retorna true se a data está entre início e fim (inclusive)
      return compareInicio >= 0 && compareFim <= 0;
    }
    
    switch (indicador) {
      case 'numero_servicos':
        final clientesUnicos = await getClientesUnicos(uid);
        // Conta serviços agendados e pagos (exceto cancelados) no período
        int count = 0;
        for (final cliente in clientesUnicos) {
          if (cliente.historicoServicos.isNotEmpty) {
            for (final entry in cliente.historicoServicos.entries) {
              final dataServicoStr = entry.key; // Formato: "dd-MM-yyyy"
              final infoServico = entry.value;
              final statusArmazenado = infoServico['statusPagamento']?.toString() ?? ClienteUnico.statusPago;
              
              // Conta apenas serviços agendados e pagos (exceto cancelados)
              if (statusArmazenado != ClienteUnico.statusCancelado) {
                try {
                  // Parse da data do serviço
                  final partesData = dataServicoStr.split('-');
                  if (partesData.length == 3) {
                    final dia = int.parse(partesData[0]);
                    final mes = int.parse(partesData[1]);
                    final ano = int.parse(partesData[2]);
                    
                    // Cria a data do serviço
                    final dataServico = DateTime(ano, mes, dia);
                    
                    // Verifica se o serviço está dentro do período da meta
                    if (estaNoPeriodo(dataServico)) {
                      count++;
                    }
                  }
                } catch (e) {
      ...
      case 'numero_clientes_planos':
        final clientes = await getClientes(uid);
        // Conta apenas clientes ativos com plano criados no período da meta
        int count = 0;
        for (final cliente in clientes) {
          if (cliente.isAtivo && cliente.planoId != null) {
            if (estaNoPeriodo(cliente.dataCadastro)) {
              count++;
            }
          }
        }
        ...
        for (final cliente in clientesUnicos) {
          if (cliente.historicoServicos.isNotEmpty) {
            for (final entry in cliente.historicoServicos.entries) {
              ...
              // Conta apenas serviços agendados e pagos (exceto cancelados) dentro do período da meta
              if (statusArmazenado != ClienteUnico.statusCancelado) {
                try {
                  // Parse da data do serviço
                  final partesData = dataServicoStr.split('-');
      ...
      case 'valor_planos':
        final clientes = await getClientes(uid);
        final planos = await getPlanos(uid);
        final planosMap = {for (var p in planos) if (p.id != null) p.id!: p};
        
        double valorTotal = 0.0;
        
        // Para cada cliente com plano, verifica pagamentos no período
        for (final cliente in clientes) {
          if (cliente.isAtivo && cliente.planoId != null && planosMap.containsKey(cliente.planoId)) {
            final plano = planosMap[cliente.planoId]!;
            
            // Gera todos os períodos até o fim do período da meta
            final todosPeriodos = gerarPeriodosHistorico(
              plano.frequencia,
              cliente.dataCadastro,
              fimPeriodo,
            );
            
            // Itera pelos períodos e verifica quais estão dentro do período da meta e foram pagos
            for (final periodo in todosPeriodos) {
              final chavePeriodo = periodo['chave'] as String;
              final dataInicioPeriodo = periodo['dataInicio'] as DateTime;
              final dataFimPeriodo = periodo['dataFim'] as DateTime;
              
              // Normaliza as datas para comparação
              final inicioPeriodoNormalizado = DateTime(inicioPeriodo.year, inicioPeriodo.month, inicioPeriodo.day);
              final fimPeriodoNormalizado = DateTime(fimPeriodo.year, fimPeriodo.month, fimPeriodo.day);
              final dataInicioPeriodoNormalizada = DateTime(dataInicioPeriodo.year, dataInicioPeriodo.month, dataInicioPeriodo.day);
              final dataFimPeriodoNormalizada = DateTime(dataFimPeriodo.year, dataFimPeriodo.month, dataFimPeriodo.day);
              
              // Verifica se o período intersecta com o intervalo da meta
              // Um período intersecta se: começa antes do fim da meta E termina depois do início da meta
              final temIntersecao = dataInicioPeriodoNormalizada.isBefore(fimPeriodoNormalizado.add(const Duration(days: 1))) &&
                                   dataFimPeriodoNormalizada.isAfter(inicioPeriodoNormalizado.subtract(const Duration(days: 1)));
              
              if (temIntersecao) {
                // Verifica se o período foi pago
                if (cliente.statusPagamento[chavePeriodo] == true) {
                  valorTotal += cliente.valor;
                }
              }
            }
          } else if (cliente.isAtivo && cliente.planoId == null) {
            // Cliente sem plano mas que pode ter pagamentos mensais anteriores
            // Itera por todos os meses dentro do período da meta
            DateTime dataAtual = DateTime(inicioPeriodo.year, inicioPeriodo.month, 1);
            final fimMesPeriodo = DateTime(fimPeriodo.year, fimPeriodo.month, 1);
            
            while (dataAtual.isBefore(fimMesPeriodo) || dataAtual.isAtSameMomentAs(fimMesPeriodo)) {
              final mesAno = DateFormat('yyyy-MM').format(dataAtual);
              
              // Verifica se o cliente pagou neste mês
              if (cliente.statusPagamento[mesAno] == true) {
                // Verifica se o cliente existia neste mês (cadastrado antes do final do mês)
                final fimDoMes = DateTime(dataAtual.year, dataAtual.month + 1, 0);
                if (cliente.dataCadastro.isBefore(fimDoMes)) {
                  valorTotal += cliente.valor;
                }
              }
