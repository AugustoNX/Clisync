...
class EvolucaoPatrimonialScreen extends StatefulWidget {
  const EvolucaoPatrimonialScreen({super.key});

  @override
  State<EvolucaoPatrimonialScreen> createState() => _EvolucaoPatrimonialScreenState();
}

class _EvolucaoPatrimonialScreenState extends State<EvolucaoPatrimonialScreen> {
  final _databaseService = DatabaseService();
  final _authService = AuthService();
  
  Map<String, double> _valoresPorMes = {};
  Map<String, int> _servicosPorTipo = {};
  Map<String, double> _valoresPorTipo = {};
  ...
  Future<void> _carregarDados() async {
    ...
    try {
      ...
      if (user != null) {
        // Busca dados do período atual (apenas clientes únicos)
        final valoresAtual = await _databaseService.getValoresPorMes(
          user.uid,
        ...
        final valoresPorTipo = await _databaseService.getValoresPorTipo(
          user.uid,
          ultimosMeses: _ultimosMeses,
        );
        ...
        setState(() {
          ...
          _valoresPorTipo = valoresPorTipo;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Widget _buildGraficoPizza() {
    ...
    for (int i = 0; i < tiposOrdenados.length; i++) {
      ...
      final valorTipo = _valoresPorTipo[entry.key] ?? 0.0;
      final cor = cores[i % cores.length];
      
      sections.add(
        PieChartSectionData(
          value: entry.value.toDouble(),
          title: '${porcentagem.toStringAsFixed(1)}%\n${NumberFormat.currency(locale: 'pt_BR', symbol: 'R\$', decimalDigits: 0).format(valorTipo)}',
          color: cor,
          radius: 80,
      ...
      children: [
        ...
        Wrap(
          ...
          children: tiposOrdenados.asMap().entries.map((entry) {
            ...
            final valorTipo = _valoresPorTipo[tipoEntry.key] ?? 0.0;
            ...
            return Row(
              ...
              children: [
                ...
                Text(
                  '${tipoEntry.key}: ${porcentagem.toStringAsFixed(1)}% (${NumberFormat.currency(locale: 'pt_BR', symbol: 'R\$', decimalDigits: 0).format(valorTipo)})',
                  style: const TextStyle(
                    color: Colors.white70,
                    fontSize: 12,
                  ),
