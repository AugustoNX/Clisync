...
class _ListaClientesUnicosScreenState extends State<ListaClientesUnicosScreen> {
  ...
  // Método auxiliar para calcular o número total de itens (clientes + anúncios)
  int _getTotalItems() {
    int totalClientes = _clientesExibidos.length;
    if (totalClientes == 0) return 0;
    
    // Calcula quantos anúncios serão exibidos
    // A cada 5 clientes, 1 anúncio
    int totalAnuncios = (totalClientes / _clientesPorAnuncio).floor();
    
    // Se tiver menos de 5 clientes, adiciona 1 anúncio no final
    if (totalClientes < _clientesPorAnuncio) {
      totalAnuncios = 1;
    }
    
    return totalClientes + totalAnuncios;
  }
  
  // Método auxiliar para determinar se o índice é um anúncio e calcular o índice do cliente
  Map<String, dynamic> _getItemType(int listIndex) {
    int totalClientes = _clientesExibidos.length;
    if (totalClientes == 0) {
      return {'isAnuncio': false, 'clienteIndex': -1};
    }
    
    // Se tiver menos de 5 clientes, o último item é anúncio
    if (totalClientes < _clientesPorAnuncio && listIndex == totalClientes) {
      return {'isAnuncio': true, 'clienteIndex': -1};
    }
    
    // Para 5 ou mais clientes: anúncios aparecem a cada 5 clientes
    // Percorre a lista simulando até chegar ao índice desejado
    int clientesMostrados = 0;
    
    for (int i = 0; i < listIndex; i++) {
      // Se já mostramos 5 clientes, o próximo item é um anúncio
      if (clientesMostrados > 0 && clientesMostrados % _clientesPorAnuncio == 0) {
        // Este é um anúncio, não conta como cliente
        continue;
      }
    ...
    // Anúncio aparece após cada grupo de 5 clientes
    if (clientesMostrados > 0 && clientesMostrados % _clientesPorAnuncio == 0) {
      return {'isAnuncio': true, 'clienteIndex': -1};
    }
    
    // Não é anúncio, retorna o índice do cliente
    return {'isAnuncio': false, 'clienteIndex': clientesMostrados};
  }
            ...
            child: _isLoading
                ...
                : RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemCount: _getTotalItems() + (_isLoadingMore ? 1 : 0),
                      itemBuilder: (context, index) {
                        ...
                        // Determina se é anúncio ou cliente
                        ...
                        // Se não é anúncio, exibe o cliente
                        final clienteIndex = itemInfo['clienteIndex'] as int;
                        if (clienteIndex < 0 || clienteIndex >= _clientesExibidos.length) {
                          return const SizedBox.shrink();
                        }

                        final cliente = _clientesExibidos[clienteIndex];
